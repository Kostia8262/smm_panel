...
const RULES = [
  ...
  [
    'publish',
    /^опубликовано:|^не ушло в |не отправлен:|публиковать некуда|с опозданием|не знаем, ушёл ли|завис в отправке|в очередь немедленно|повтор|подменено ссылок|снят из сети|после ручной проверки|замечание к посту|обновлён в /,
  ],
  [
    'system',
  ...
  ['post', /создан пост|^пост #\d+|миниатюра отклонена|добавлен тренд|тренд-аккаунт/],
  [
    'access',
