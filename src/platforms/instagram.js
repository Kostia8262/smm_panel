...
export function isConfigured() {
  return Boolean(process.env.INSTAGRAM_USER_ID && process.env.FACEBOOK_PAGE_TOKEN);
}

export function missingConfig() {
  const missing = [];
  if (!process.env.INSTAGRAM_USER_ID) missing.push('INSTAGRAM_USER_ID');
...
export async function check() {
  ...
  const res = await fetch(`${API}/${user}?fields=username,followers_count&access_token=${token}`);
...
export async function remainingQuota() {
  const token = process.env.FACEBOOK_PAGE_TOKEN;
  const user = process.env.INSTAGRAM_USER_ID;
  const res = await fetch(`${API}/${user}/content_publishing_limit?access_token=${token}`);
  const data = await res.json();
  if (data.error) throw new Error(data.error.message);
...
async function waitReady(containerId, { tries = 30, pauseMs = 10000 } = {}) {
  const token = process.env.FACEBOOK_PAGE_TOKEN;
...
export async function publish({ text, media = [], formatId = 'feed-portrait', publicUrl }) {
  const user = process.env.INSTAGRAM_USER_ID;
  ...
  if (media.length === 1) {
    ...
    ({ id: containerId } = await call(`${user}/media`, clean(params)));
  } else {
    const children = [];
  ...
  if (media.some((m) => m.kind === 'video') || isReel) await waitReady(containerId);

  const published = await call(`${user}/media_publish`, { creation_id: containerId });
