...
export async function publishPost(postId) {
  ...
  for (const target of post.targets) {
    ...
    if (!adapter.isConfigured()) {
      const missing = adapter.missingConfig().join(', ');
      markFailed(target, `Не настроено: ${missing}`);
    ...
    try {
      ...
      const out = await adapter.publish({ text, media, formatId: target.format_id, publicUrl });
      db.prepare(
