...
export const PLATFORMS = {
  ...
  tiktok: {
    ...
    accent: '#FE2C55',
    order: 5,
    // Адаптер настоящий с 14.09.2026 (сверен с документацией, тесты на всё);
    // «не подключён» в проверке поста врало бы: очередь публикует.
    ready: true,
    text: {
    ...
    // Режим «сразу в ленту» или «в черновики» — не раскладка, а настройка цели
    // (src/target-options.js): кадр и лимиты у них одинаковые.
    formats: [
      {
        id: 'video',
        title: 'Видео',
        ...
        role: 'reel',
        vertical: true,
        // Через API — до 10 минут; точный предел аккаунта даёт creator_info.
        media: { kinds: ['video'], groupMax: 1, required: true, video: { minSeconds: 3, maxSeconds: 600 } },
      },
      {
        id: 'photo',
        title: 'Фото-пост',
        ...
        h: 1920,
        role: 'feed',
        // Фото TikTok забирает только по ссылке с подтверждённого адреса панели.
        media: { kinds: ['image'], groupMax: 35, required: true },
      },
    ],
    safeZones: {
      reel: [
    ...
    notes: [
      'Пока приложение не прошло аудит TikTok, посты видны только вам («Только я»).',
      'Кто увидит пост, комментарии, дуэты и стичи выбираются у каждого поста — TikTok запрещает выбирать за человека.',
      'Звук через API не выбрать: нужен трендовый звук — отправляйте ролик в черновики и добавляйте звук в приложении.',
      'Фото-посты — только после подтверждения адреса панели в кабинете TikTok.',
    ],
  },
};
