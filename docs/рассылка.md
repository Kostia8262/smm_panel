...
ни ключей. Образцы для тестов — выдуманные адреса.

**Состояние на 14.09.2026:** фаза 1 «Базы» выкачена (`30faf32`, `7c2094a`,
`2f577ae`). Фаза 2 «Ящик и отписка» выкачена (`9dcadea`), ящик подключён
кнопкой на проде, пробное письмо дошло; фирменный шаблон письма — `7050ab6`.
Фаза 3 «Письмо» сделана 14.09: вкладка «Письма», редактор блоками, живой
предпросмотр, проверки, пробное письмо версии, согласование (`16fe7bd`, выкачена).
Фаза 4 «Отправка» сделана 14.09: расписание и «разослать сейчас», снимок
получателей, отправщик в воркере, пауза/продолжение/отмена, прогресс и прогноз,
письма поимённо, снятие картинок (`f8dd78e`, выкачена). Фаза 5 «Результаты» и
из фазы 6 — возвраты и частота — сделаны 14.09. Ящик-отправитель:
обычный @gmail.com (адрес в репозиторий не пишем — он придёт в базу при
подключении кнопкой), потолок Google 500 получателей за 24 часа. Google Cloud
настроен владельцем: Gmail API, три права, клиент Web, In production.

Отличия от первоначального плана фазы 2: `makeState`/`readState` не вынесены в
`src/oauth/state.js` — они уже общие в `oauth/threads.js` (площадка передаётся
параметром); сторож ящиков — `sweepSenders` в `sender/senders.js`, зовётся из
обхода токенов воркера; тревога о мёртвом ящике — заметка на вкладке «Базы»,
а не значок в меню.
...
1. **Приём.** Файл до 20 МБ (CSV, TSV, TXT, XLSX, VCF) или вставленный текст —
   ...
   через табуляцию. Исходный файл лежит в `mail_imports.source` только до
   подтверждения или отказа (не дольше суток): без него не сменить кодировку,
   разделитель или лист, не загружая файл заново.
...
4. **Колонки.** Заголовок — первая строка без адресов, если ниже адреса есть.
   ...
   же именем. Колонка адреса пуста, а адрес уехал в соседнюю колонку — он
   берётся с пометкой «адрес не в своей колонке».
...
6. **Проверка каждого адреса** — вердикт и причины:
   - `invalid` — синтаксис; кириллица в имени ящика; домен без MX и без A;
   - `fixable` — предложение исправления, **не молчаливая замена**:
     - опечатка в популярном домене (`gmial.com`, `gmail.con`, `ukr.nte`);
     - **кириллические двойники латиницы** (`gmаil.com` с русской «а» — частая
       беда баз, набранных руками); в предпросмотре русские буквы подсвечены,
       иначе «исправить на то же самое» не понять;
     - пробел внутри адреса — и **пробел, разрезавший имя ящика**:
       `lina danyliuk@gmail.com` без этой проверки находился как готовый
       `danyliuk@gmail.com`, то есть чужой ящик (найдено на осмотре 13.09);
   ...
   Домен, который не ответил (таймаут, сбой резолвера), — «не знаем», а не
   «почты нет»: адрес остаётся, в предпросмотре — «N доменов не ответили».
...
Сделано в фазе 3 (`compose/render.js`): `hero` (главная картинка 1200×600, одна,
первой) · `eyebrow` · `heading` · `subheading` · `text` (разметка только
`**жирный**`, `_курсив_`, `[ссылка](https://…)` — не HTML; пустая строка —
абзац) · `bullets` (пункт — строка) · `button` (надпись, адрес, подпись) ·
`image` (1200×675, alt, ссылка) · `callout` (заголовок, текст) · `divider`.
Неизвестные типы и поля отбрасывает `sanitizeBlocks`, не больше 40 блоков.
Новое письмо создаётся с образца (`sampleBlocks`).
...
«Имя» над текстом; любая другая подстановка — блокер. Имя ставим в начало фразы:
склонять украинские имена в звательный падеж панель не умеет.
...
- *(перенесено в фазу 4)* Ссылки на свои домены (`settings.own_domains`) заменяются короткими `/r/` с
...
- Сжатие в браузере (`views/mail/editor.js`, `prepareImage`): до 1 200 px по
  ширине, JPEG с качеством 0,85 на белом фоне; PNG с прозрачностью остаётся PNG.
  JPEG/PNG не шире 1 200 px и не тяжелее 400 КБ уходит как есть. WebP и прочее
  браузер перекодирует в JPEG. Сервер принимает только JPEG и PNG по сигнатуре,
  до 2 МБ (`MAIL_IMAGE_LIMITS`), и сверяется с общей квотой. Отдельных миниатюр
  нет — редактор показывает сам файл.
...
- **Файл живёт, пока нужен хотя бы одной незавершённой рассылке** (от `draft` до
  `paused`). Снятие — фаза 4. Рассылка завершена или отменена → через 10 минут файл снимается,
...
панель. Картинки письма в предпросмотре — `data:`-адресами (`https:` — только
логотип школы с сайта). Рамка `sandbox="allow-same-origin"` без скриптов: тот же
источник нужен, чтобы измерить высоту письма и держать одну прокрутку.

Режимы: компьютер — рамка 680 px, телефон — 390 px; узкая колонка уменьшает
рамку масштабом, а не сужает письмо (иначе «компьютер» показал бы мобильную
вёрстку). С телефона предпросмотр открывается в режиме «Телефон». Строка «как во
входящих» — отправитель, тема, прехедер; поле «Имя получателя в примере».
Отпечаток и первая база — в `v=` адреса: рамка перезагружается только при
изменении письма.

### 9.5 Проверки — `checkCampaign` в `campaigns.js`, до утверждения и постановки в очередь

Сделано в фазе 3, кроме прогноза длительности (фаза 4). Пробное письмо версии
проверяет не `checkCampaign`, а `readyOrThrow` — при утверждении и отправке на
согласование. Сверх списка: блок картинки без файла, кнопка без ссылки,
пропорции картинки мимо шаблона больше 8 % (предупреждение), база в архиве.
...
Сделано в фазе 4: `src/mail/runner.js` (действия, прогноз, снимок, шаг, разбор
зависших, снятие картинок) и `src/mail/sender/throttle.js` (окно по поясу,
случайная пауза, прогноз). Отличия от плана: короткие ссылки заводятся **в
момент старта** и лежат в `mail_campaigns.link_map` (пробное письмо уходит с
прямыми ссылками); ящик, придержанный Google, — `mail_senders.paused_until` +
`hold_reason`; повтор 5xx — `mail_sends.not_before` (2, 4 минуты); стоп-лист
(`store.suppress`) сразу переводит `queued` этого адреса в `skipped`, стирание —
заменяет адрес в `mail_sends` на `erased-ID`. Пауза и продолжение — СММщику
тоже; отмена и «разослать сейчас» — владельцу.
...
Всё — в пределах школы из параметра `project`; маршруты — `src/mail/routes.js`.

```
Базы — сделано в фазе 1
GET    /api/mail/summary                     mail: сводка школы, базы со счётчиками,
                                             незавершённые загрузки (только владельцу)
POST   /api/mail/lists                       новая база
PUT    /api/mail/lists/:id                   название, описание, основание, { archived }
GET    /api/mail/contacts                    ?list=ID|all&q=&status=&sort=&page=
GET    /api/mail/export.csv                  ?list=ID|all
POST   /api/mail/contacts                    { listId, email, name } — вручную
GET    /api/mail/contacts/:id                карточка: базы, основание, стоп-лист
PUT    /api/mail/contacts/:id                { name, email } (+ { listId, attrs })
POST   /api/mail/contacts/bulk               { ids, action: remove|copy|move|unsubscribe|bounced,
                                               listId, targetListId, note }
POST   /api/mail/contacts/:id/resubscribe    { note } — обязательно
POST   /api/mail/contacts/:id/erase
POST   /api/mail/imports                     multipart: file (+ pasted=1 для текста), listId
GET    /api/mail/imports/:id                 ?verdict=&page=&listId=ID|new&warnings=0|1
                                             → разбор, счётчики, строки, прогноз
PUT    /api/mail/imports/:id                 { encoding, delimiter, sheet, hasHeader, roles }
PUT    /api/mail/imports/:id/decisions       { rows: [{ seq, decision }] } | { all }
POST   /api/mail/imports/:id/commit          { listId | newList, includeWarnings }
DELETE /api/mail/imports/:id                 отказ: файл и строки предпросмотра стираются

Ящики — сделано в фазе 2 (mail_senders, только владелец)
GET    /api/mail/senders                     приложение Google (без секрета), ящики с потолком и счётчиком
GET    /api/mail/senders/alerts              mail: есть ли мёртвый ящик — для заметки на «Базах»
PUT    /api/mail/google-app                  { clientId, clientSecret } — пустой секрет не затирает сохранённый
POST   /api/mail/senders/oauth/start         { loginHint } → { url }
GET    /oauth/google                         возврат из окна Google (страница, редирект на #/mail/senders)
PUT    /api/mail/senders/:id                 { displayName, dailyCap, warmup, windowFrom, windowTo }
POST   /api/mail/senders/:id/check           свежий access-токен, состояние ящика
POST   /api/mail/senders/:id/test            { to: [...] } ≤ 5, пусто — себе
DELETE /api/mail/senders/:id                 отозвать токен у Google, отметить отключённым

Письма — сделано в фазе 3 (mail; утверждение и возврат — mail_senders, то есть владелец).
Ответ правки и действий — { campaign, check, options }: письмо, проверка с
аудиторией, базы и ящики для выбора
GET    /api/mail/campaigns                   список: состояние, базы, получат, пробное этой версии
POST   /api/mail/campaigns                   новая с образца или копия { fromId }
GET    /api/mail/campaigns/:id
PUT    /api/mail/campaigns/:id               { title, subject, preheader, fromName, replyTo, senderId,
                                               blocks, lists: { include, exclude } }
DELETE /api/mail/campaigns/:id               мягкое удаление; в отправке — нельзя
POST   /api/mail/campaigns/:id/media         multipart: file → { media }
GET    /api/mail/media/:name                 картинка для редактора
GET    /api/mail/campaigns/:id/preview.html  ?name=&v= — своя CSP, SAMEORIGIN
POST   /api/mail/campaigns/:id/test          { to: [...] }, не больше 5 адресов
POST   /api/mail/campaigns/:id/submit        СММщик → review; владелец или без согласования → approved
POST   /api/mail/campaigns/:id/approve       владелец
POST   /api/mail/campaigns/:id/reject        владелец, { note } — обязательно

Отправка — сделано в фазе 4. Ответ — тот же { campaign, check, options } плюс
delivery: прогноз, окно, остаток потолка, пауза Google
POST   /api/mail/campaigns/:id/schedule      { at: ISO } — время; { at: null } — разослать сейчас (владелец)
POST   /api/mail/campaigns/:id/unschedule    назад в «утверждено»
POST   /api/mail/campaigns/:id/pause
POST   /api/mail/campaigns/:id/resume
POST   /api/mail/campaigns/:id/cancel        владелец
GET    /api/mail/campaigns/:id/sends         mail_contacts: ?status=&page= — письма поимённо
...
| Адрес | Экран |
|---|---|
| `#/mail` | **Письма** (фаза 3) — вкладки «Письма / Базы / Ящики»; строка письма: название и тема, состояние словом, «пробное этой версии ушло», базы, получат, копия, убрать. В фазе 4 — прогресс «1 240 из 3 000» и прогноз окончания |
| `#/mail/lists` | **Базы** школы (фаза 1; до фазы 3 жили на `#/mail`); тревога, если ящик мёртв |
| `#/mail/lists/:id`, `#/mail/lists/all` | **Содержимое базы** или все контакты школы; фильтр в адресе; карточка контакта над таблицей |
| `#/mail/import/new?list=ID` | **Загрузка** — файл или вставленный текст |
| `#/mail/import/:id` | **Предпросмотр** — как прочитан файл, вердикты, строки, «куда добавить» с прогнозом |
| `#/mail/letter/:id` | **Редактор** (фаза 3) — сверху состояние и три шага (без ошибок → пробное этой версии → утверждено), «Вернуть на доработку»; слева «Конверт», «Получатели» с расчётом, «Содержание» блоками, «Проверка» (блокеры отдельно от предупреждений, вес); справа прилипший предпросмотр. В шапке — «Пробное письмо» и «Утвердить» / «На согласование». Правки сохраняются сами через 0,7 с, форма не перерисовывается |
| `#/mail/senders` | **Отправители** (фаза 2) — ящик, состояние токена, отправлено за 24 ч из потолка, прогрев, окно, приложение Google |
...
### Фаза 2 — Ящик и отписка · сделана 13.09.2026

**Внутри:** миграция `025-mail-senders`; `sender/google-oauth.js`, `gmail.js`,
`senders.js` (приложение, ящики, потолки, access-токен, сторож, пробное
письмо); `compose/mime.js`; `unsubscribe.js` + `public/css/unsubscribe.css`;
`deriveKey` в `secrets.js`; вкладки «Базы / Ящики», экран `views/mail/senders.js`.
Тесты — `test/mail-senders.test.js`.
...
### Фаза 3 — Письмо · сделана 14.09.2026

**Внутри:** таблицы фазы 3; блоки, сборщик, подвал из подписи проекта,
подстановки; картинки (сжатие, `Content-ID`, хранение); ~~короткие ссылки~~ (в
фазу 4 — им нужен номер рассылки в utm); предпросмотр со своей CSP; проверки;
пробная отправка версии; согласование.

Сделано: миграция `027-mail-campaigns` (`mail_campaigns`, `mail_campaign_lists`,
`mail_media`); `src/mail/campaigns.js`, `src/mail/compose/media.js`;
`sendTestMessage` в `senders.js` (пробное письмо любой вёрстки с отпечатком
версии); маршруты в `routes.js`; экраны `views/mail/campaigns.js` и
`views/mail/editor.js`. Тесты — `test/mail-campaigns.test.js`. Картинки письма
пока не снимаются вовсе — снятие вместе с завершением рассылки в фазе 4.
...
### Фаза 4 — Отправка · сделана 14.09.2026

Сделано: миграция `028-mail-sends`; `runner.js`, `sender/throttle.js`; счётчик
суток ящика учитывает `mail_sends` (`sent`, `unknown`, `sending`); цикл
`mailTick` в `queue/worker.js`; маршруты отправки; блок «Отправка» в редакторе
(время, «Разослать сейчас», прогресс, что держит отправку, пауза, отмена,
письма поимённо для владельца), прогресс и прогноз в списке писем, обновление
раз в 10–15 секунд. Тесты — `test/mail-sending.test.js`. Повтор писем с
неизвестной судьбой владельцем — сделан в фазе 5 (`retryUnknown`).
...
### Фаза 5 — Результаты · сделана 14.09.2026

**Внутри:** отчёт рассылки; история писем в карточке контакта; заявки по
`utm_campaign`; события в журнале.

Сделано: `src/mail/report.js` — `campaignReport` (ушло → переходы → заявки,
отписки по письму, причины «не ушло», ссылки подписаны тем, на что нажимали:
«Кнопка «…»», «Ссылка «…»»; имена из заявок — только владельцу) и `retryUnknown`
(повтор писем с неизвестной судьбой — владелец, с предупреждением о дублях;
разосланная рассылка возвращается в отправку). `GET .../report`,
`POST .../retry-unknown`. Панель «Результаты» в редакторе грузится один раз и
по кнопке «Обновить» (ходит в админку школы за заявками). В карточке контакта —
«Письма»: что получал и «отписался по этому письму». Тесты —
`test/mail-results.test.js`.
...
- **Возвраты — сделано 14.09.** «Базы» → «Возвраты писем»: вставить текст писем
  «не доставлено» → `src/mail/bounces.js` находит адреса своих контактов
  (служебные и ящики рассылки отбрасывает, показывает последнее письмо) → после
  подтверждения «адреса нет» (стоп-лист). Автоматический разбор ящика — только если
  возвратов станет много: для него нужно отдельное, более широкое право Google на
  чтение почты.
- **Частота — сделано 14.09**: не больше одного письма человеку от школы за N
  дней (0–60, настройка `mail_gap_days_<проект>`, меняет владелец на «Письмах»).
  Расчёт аудитории показывает «получали недавно» и предупреждает (не запрещает:
  письмо можно запланировать позже); снимок кладёт таких в `skipped` с причиной.
- **Сегменты** по колонкам базы («курс = дизайн»).
- **Форма подписки на сайтах — сделано и выкачено 14.09** (`src/mail/signups.js`,
  миграции `029-mail-signups`, `030-mail-signup-consent`).
  - **Без подтверждения по письму** (решение владельца): `POST /s/subscribe` — новый
    адрес сразу в базе «Подписка с сайта» (основание `form`) + приветственное письмо
    с отпиской. Подтверждение кнопкой (`GET/POST /s/confirm/:токен`) — только адресам,
    которые отписывались, стёрты или не принимали письма: иначе чужая форма
    возвращала бы им рассылку. Ответ формы одинаков в обоих случаях.
  - Поля: email, school, page, website (ловушка), **consent_version** (обязателен,
    код текста согласия у формы, `footer-2026-09-uk|ru`), lang, utm_source/medium/
    campaign, referrer, landing_path (из `window.Attribution` сайта — метка первого
    захода, как у заявок; поля для моста «CRM школы ↔ панель»).
  - Фильтры без подтверждения: опечатка в домене → `typo`+suggestion, домен без
    почты (MX) → `invalid_email`, 5 попыток в час с адреса обращающегося, 3 записи
    адресу в сутки и не чаще раза в 10 минут, **суточный предел школы 60** — сверх
    него `held` без писем, тревога в журнал на 25 и 60.
  - CORS — только свои домены (`own_domains`); `Origin: null` встроенных браузеров —
    по Referer. **Доверенный прокси** сайта (запрос с адреса из `signup_proxy_ips`,
    по умолчанию сервер сети и localhost) передаёт `X-Subscribe-Client-Ip` и
    `X-Subscribe-Origin` — так у child лимит попыток у каждого посетителя свой.
  - Ответ несёт `code` (accepted, typo, invalid_email, consent_missing, too_many,
    unavailable, send_failed, foreign_site, unknown_school, error) — сайт переводит
    по нему. Письма уходят через `sendTestMessage` (в счёт потолка ящика, без окна).
  - **Блок в подвалах** (репозитории сайтов): хаб mycomputer.education (10 подвалов),
    python, roblox, minecraft, webdesign, mycomputer.school и лендинги .school, child (Django, прокси
    `/api/subscribe` рядом с оплатой), FluentFox (Tailwind, генераторы). Node-сайтам —
    строка `https://smm.mycomputer.education` в `connectSrc`; FluentFox — `.htaccess`.
    Политики конфиденциальности — раздел о рассылке (7-2, у FluentFox 7-1).
    python, roblox, minecraft — добавлены 14.09 по решению владельца (на замер не влияет).
    Галочка согласия в формах заявок — не сделана.
- **Лента согласий CRM школы → базы — приёмник сделан 14.09** (`src/mail/crm-feed.js`,
  `test/mail-crm-feed.test.js`); эндпоинт в админке — фаза 3 вкладки админки, **пока его нет**.
  - Контракт: `GET /api/integration/mail/feed?after=<seq>&limit=500`, `x-integration-key`
    **отдельного ключа со скоупом `mail:feed`** («SMM-панель — стрічка»; настройка
    `crm_feed_token`, шифротекстом; поле «Ключ ленты согласий» и «Проверить ленту» в
    «Связь с заявками школы»). Ответ `{ items: [{ seq, email, consent: {at, version,
    source}|null, person: {kind, id, childName, status, course, site, lang}|null }], next,
    hasMore }`. Лента по адресам: адрес в пачке один раз, с текущим состоянием.
  - Воркер (`feedTick`, раз в минуту, свой флаг занятости) дочитывает пачки, пока
    `hasMore`; пачка и курсор `crm_feed_cursor` — одной транзакцией. `after=0` —
    полная пересверка, повтор не плодит событий.
  - Согласие есть → база «CRM школы — заявки» (`consent.source = thank_you`, основание
    `lead`) или «CRM школы — клиенты» (`manager`, `client`) своей школы; колонки CRM id,
    Кто, Ребёнок (`childName`: **имя ребёнка — в имя контакта не кладётся**, родителя в CRM нет), Статус, Курс, Сайт, Язык — под будущие сегменты. Школа по `person.site`:
    child.mycomputer.education → child, `*.mycomputer.education` и пусто → education,
    `*.mycomputer.school` → school, fluent-fox.site → fluentfox; чужой домен — не берём.
  - **`consent: null` → адрес уходит из баз ленты (removed_at), но НЕ в стоп-лист**:
    снятая галочка чаще опечатка, чем «не пишите». Стоп-лист — только отписка в панели.
  - Отказавшегося (стоп-лист, статус не active) лента не возвращает. Заявка стала
    клиентом — адрес переезжает из базы заявок в базу клиентов. AAA /gift в ленту не
    пишутся вовсе (подтвердила админка), их заявки без согласия — `consent: null`.
  - Итог захода — `crm_feed_last_at|last_error|last_stats`, строка под полем ключа;
    в журнал — только когда что-то пришло или не взято.
  - Лента на проде админки с 14.09 (`94100101`). Тексты согласий —
    `GET /api/integration/mail/consent-texts` (тот же ключ): кэш `crm_consent_texts`,
    перечитывается при незнакомом коде в пачке (не чаще раза в 10 минут), старые коды не
    теряются. Карточка контакта показывает у базы источник («лента CRM школы» / «форма на
    сайте»), код и дату согласия и сам текст — доказательство согласия.
...
3. Содержимое баз, загрузку и выгрузку видит только владелец; СММщик — названия и
   размеры баз. *(фаза 1)*
4. Под рассылку — отдельный ящик, не основной рабочий. **Выбран обычный
   @gmail.com (13.09.2026)** — потолок 500, External + In production. *(фаза 2)*
