...
- **Форма подписки на сайтах — приём сделан 14.09** (`src/mail/signups.js`,
  миграция `029-mail-signups`): `POST /s/subscribe` (email, school, page, website —
  ловушка) → письмо с кнопкой → `GET /s/confirm/:токен` показывает страницу,
  `POST` подписывает в базу «Подписка с сайта» (основание `form`) и снимает прежнюю
  отписку. CORS — только свои домены; `Origin: null` встроенных браузеров — по
  Referer. Ответ несёт `code` (accepted, typo+suggestion, invalid_email, too_many,
  unavailable, send_failed, foreign_site, unknown_school, error) — сайт переводит
  по нему. Ответ не выдаёт, подписан ли адрес. Лимиты: 5 попыток в час с адреса
  обращающегося, 3 письма адресу в сутки, не чаще раза в 10 минут; токен 7 дней,
  в базе хеш. Письмо-подтверждение уходит через `sendTestMessage` (в счёт потолка
  ящика, без окна отправки и без List-Unsubscribe).
  **Блок в подвале** — репозиторий сайтов, `sites/main`: `.footer__subscribe` над
  возрастными подборками, `/js/subscribe.js`; пока только на главной хаба, на dev.
  Хаб: CSP `connect-src *.mycomputer.education` пускает fetch, `form-action 'self'` —
  обычную форму нет. На сателлитах и .school CSP чужих доменов не пускает — для
  раскатки нужен свой прокси `/api/…` на каждом сайте или правка CSP. Галочка
  согласия в формах заявок — не сделана.
