 ...
 * @param {{staleMinutes?: number, now?: string}} opts
 * @returns {{unknown: number, resumed: number}}
 */
export function recoverStuck(db, log, { staleMinutes = STALE_MINUTES } = {}) {
  const cutoff = `-${staleMinutes} minutes`;

  // 1. Цели, застрявшие в момент отправки: судьба неизвестна.
  const hanging = db
    .prepare(
      `SELECT t.id, t.post_id, t.platform
       FROM post_targets t
  ...
  const markUnknown = db.prepare(
    `UPDATE post_targets
     SET status = 'needs_check',
         error = 'Отправка прервалась — неизвестно, вышел ли пост. Проверьте канал вручную.'
     WHERE id = ?`
  );
  for (const row of hanging) {
    markUnknown.run(row.id);
